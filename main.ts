...
const dir = Deno.readDir(config.source);
...
for await (const file of dir) {
  ...
  if (matches === null) {
    throw new Error(`Invalid file name: "${file.name}"; was expecting a name and an index within brackets. Example: Company Name (3) or Cool restaurant (9)`);
  }
  ...
  if (isNaN(index)) {
    throw new Error(`Invalid file index: "${index}"; was expecting a whole integer like 1 or 4`);
  }
...
for (const [group, indices] of groupIndices) {
  ...
  if (max !== sorted.length) {
    throw new Error(`Group "${group}" has a wrong number of files! Expected ${sorted.length} files, but ${max} were found.`);
  }

  const merger = new PDFMerger();

  for (const index of sorted) {
    await merger.add(`${config.source}/${group} (${index}).pdf`);
  }

  await merger.save(`${config.target}/${group}.pdf`);
